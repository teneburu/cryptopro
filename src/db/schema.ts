import { date, pgTable, serial, text, pgEnum, foreignKey } from "drizzle-orm/pg-core";
import { z } from "zod";
...
export const FormSchema = z.object({
  ...
  email: z.string({message: "Veuillez renseigner votre adresse email."})
    .email(),
  invitecode: z.string({message: "Veuillez renseigner votre code d'invitation. Si vous n'en disposez pas, contactez nous."})
    .min(10)
    .max(10),
}).required();
